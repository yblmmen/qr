...
while($row = sql_fetch_array($result)) {
	$list[$i]['번호'] = $i;
	$list[$i]['자산번호'] = $row['wr_subject'];
	$list[$i]['모델명'] = $row['wr_content'];
	$list[$i]['제조사'] = $row['wr_1'];
	$list[$i]['시리얼'] = $row['wr_4'];
	$list[$i]['기타'] = $row['wr_2'];
	$i++;
...
<table border=0 cellpadding=0 cellspacing=0 width=1094 class=xl6328651 style='border-collapse:collapse;table-layout:fixed;width:822pt'>
	...
	<tr height=21 style='mso-height-source:userset;height:15.95pt'>
		<td height=21 class=xl6828651 width=42 style='height:15.95pt;width:32pt'>번호</td>
		<td class=xl6828651 width=100 style='border-left:none;width:100pt'>자산번호</td>
		<td class=xl6828651 width=116 style='border-left:none;width:116pt'>모델명</td>
		<td class=xl6828651 width=133 style='border-left:none;width:133pt'>제조사</td>
		<td class=xl6828651 width=133 style='border-left:none;width:133pt'>시리얼</td>
		<td class=xl6828651 width=300 style='border-left:none;width:300pt'>기타</td>
	</tr>
<?php foreach($list as $row) { ?>
	<tr height=21 style='mso-height-source:userset;height:15.95pt'>
		<td height=21 class=xl6428651 style='height:15.95pt;border-top:none'><?php echo $row['번호'] ?></td>
		<td class=xl6528651 style='border-top:none;border-left:none'><?php echo $row['자산번호'] ?></td>
		<td class=xl6628651 style='border-top:none;border-left:none'><?php echo $row['모델명'] ?></td>
		...
		<td class=xl6428651 style='border-top:none;border-left:none'><?php echo $row['기타'] ?></td>
	</tr>
<? } ?>
