...
if ($w == '' || $w == 'r') {
    ...
    // 새글 INSERT
    if($is_new) sql_query(" insert into {$g5['board_new_table']} ( bo_table, wr_id, wr_parent, bn_datetime, mb_id, as_reply, as_re_mb,wr_1,wr_2,wr_3 ) values ( '{$bo_table}', '{$wr_id}', '{$wr_id}', '".G5_TIME_YMDHIS."', '{$member['mb_id']}', '{$wr_reply}', '{$as_re_mb}','{$wr_11}','{$wr_13}','검토대기'  ) ");
	// 결재 등록 알림
	sql_query("insert into {$g5['apms_response']} ( bo_table, wr_id, mb_id,my_id,comment_cnt,regdate,type,subject,my_name) values ( '{$bo_table}','{$wr_id}','{$wr_11}', '{$member['mb_id']}', '1','".G5_TIME_YMDHIS."','2','검토할 문서가 등록되었습니다','{$member['mb_name']}') ");
	sql_query(" update {$g5['member_table']} set as_response = as_response + 1 where mb_id = '{$wr_11}' ");
    // 게시글 1 증가
    ...
    sql_query($sql);
    $sqlnew = " update {$g5['board_new_table']}
                set  wr_1 = '{$wr_11}',
                     wr_2 = '{$wr_13}',
                     wr_3 = '검토대기',
                     bn_datetime = '".G5_TIME_YMDHIS."'
		             where wr_id = '{$wr['wr_id']}' and bo_table='{$bo_table}'";
    sql_query($sqlnew);
...
if ($file_upload_msg)
    alert($file_upload_msg, G5_HTTP_BBS_URL.'/board.php?bo_table='.$bo_table.'&wr_id='.$wr_id.'&wr_datetime='.$wr_datetime);
else
    goto_url(G5_HTTP_BBS_URL.'/board.php?bo_table='.$bo_table.'&wr_id='.$wr_id.'&wr_datetime='.$wr_datetime);
?>
