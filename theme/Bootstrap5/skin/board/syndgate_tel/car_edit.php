...
if ($mode) {
    switch ($mode) {
        ...
        case "save":
            ...
            // 회원 인증 후 페이지 접근
            if (!checkMemberAuthentication($member['mb_id'])) {
                header('Location: '.G5_URL.'/login.php'); // 비회원이 접근한 경우 로그인 페이지로 리다이렉트
                exit;
            }
...
<div id="mem_edit">
	...
	<table>
		...
		<tr>
			<th>시리얼</th>
			<td>
				<textarea name="wr_1" class="mb_content" rows="5"><?php echo $rs['wr_1']; ?></textarea>
			</td>
		</tr>
		<tr>
			<th>상세정보</th>
