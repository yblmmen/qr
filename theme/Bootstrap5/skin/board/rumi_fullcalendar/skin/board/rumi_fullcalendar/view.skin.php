...
include_once($board_skin_path.'/function.lib.php'); // 공통 함수 모음
...
if($view['wr_5']) { // "종일" 일정이면 "년월일"만 표시
    $wr_1 = date("Y년 m월 d일", strtotime($view['wr_1']));
    $wr_2 = date("Y년 m월 d일", strtotime($view['wr_2']));
    $startYoilA = get_yoil($view['wr_1']);
    $endYoilA = get_yoil($view['wr_2']);
} else {
    $wr_1 = date("Y년 m월 d일 H시 i분", strtotime($view['wr_1']));
    $wr_2 = date("Y년 m월 d일 H시 i분", strtotime($view['wr_2']));
    $startYoilA = get_yoil($view['wr_1']);
    $endYoilA = get_yoil($view['wr_2']);
}

// 반복일정인 경우 추가.
if($view['wr_6'] > 0 && $start && $end) {
    if($view['wr_5']) {
        $startDate = date("Y년 m월 d일", strtotime($start));
        $endDate = date("Y년 m월 d일", strtotime($end));
        $startYoilB = get_yoil($start);
        $endYoilB = get_yoil($end);
    } else {
        $wr_1_time = date("H시 i분", strtotime($view['wr_1']));
        $wr_2_time = date("H시 i분", strtotime($view['wr_2']));
        $startDate = date("Y년 m월 d일", strtotime($start))." ".$wr_1_time;
        $endDate = date("Y년 m월 d일", strtotime($end))." ".$wr_2_time;
        $startYoilB = get_yoil($start);
        $endYoilB = get_yoil($end);
    }
}
...
// 일정관리에서는 사용하지 않음.
// $prev_href = $board_skin_url."/view.php?bo_table=".$bo_table."&wr_id=".$prev['wr_id']; // 이전글
// $next_href = $board_skin_url."/view.php?bo_table=".$bo_table."&wr_id=".$next['wr_id']; // 다음글
...
<article id="bo_v" class="<?php echo $popupPadding; ?>">
    <header>
        <h2 id="bo_v_title">
...
;
    ...
    <section id="bo_v_atc">
        ...
        <div class="bo_w_tit write_div rumi-item">
            <span class="rumi-item-title">일정</span>
            <span class="rumi-item-value pl-10">
                <?php echo $wr_1." ({$startYoilA}) ~ ".$wr_2." ({$startYoilA})"; ?>
            </span>
        </div>

        <?php if($view['wr_6'] > 0) { ?>
        <div class="bo_w_tit write_div rumi-item">
            <span class="rumi-item-title">반복일정</span>
            <span class="rumi-item-value pl-10">
                <?php echo $startDate." ({$startYoilB}) ~ ".$endDate." ({$startYoilB})"; ?>
            </span>
        </div>
        <div class="bo_w_tit write_div rumi-item">
            <span class="rumi-item-title">반복구분</span>
            <span class="rumi-item-value pl-10">
                <?php echo ($view['wr_8'] > 0) ? str_replace("N", $view['wr_8'], $repeatArr[$view['wr_6']]) : $repeatArr[$view['wr_6']]; ?>
            </span>
        </div>
        <div class="bo_w_tit write_div rumi-item">
            <span class="rumi-item-title">반복종료일</span>
            <span class="rumi-item-value pl-10">
                <?php echo date("Y년 m월 d일", strtotime($view['wr_7'])); ?>
            </span>
        </div>
        <?php } ?>
