...
class TCPDF_STATIC {
	...
	* @since 5.9.121 (2011-09-28)
	...
	public static function extractCSSproperties($cssdata) {
		...
		$cssdata = preg_replace('/@media[\s]+([^\{]*)\{/i', '@media \\1짠', $cssdata);
		$cssdata = preg_replace('/\}\}/si', '}짠', $cssdata);
		// trim string
		$cssdata = trim($cssdata);
		// find media blocks (all, braille, embossed, handheld, print, projection, screen, speech, tty, tv)
		$cssblocks = array();
		$matches = array();
		if (preg_match_all('/@media[\s]+([^\짠]*)짠([^짠]*)짠/i', $cssdata, $matches) > 0) {
			foreach ($matches[1] as $key => $type) {
				$cssblocks[$type] = $matches[2][$key];
			}
			// remove media blocks
			$cssdata = preg_replace('/@media[\s]+([^\짠]*)짠([^짠]*)짠/i', '', $cssdata);
		}
